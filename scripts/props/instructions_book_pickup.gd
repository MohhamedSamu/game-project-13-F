...
@export var use_prompt: String = "[E] Leer instrucciones"
...
@export var hold_rotation_deg: Vector3 = Vector3(-35.0, 180.0, 12.0)

@export_group("Al soltar")
@export var drop_forward_speed: float = 0.9
@export var drop_up_speed: float = 0.2
@export var drop_spawn_forward: float = 0.32
@export var drop_spawn_lift: float = 0.06
...
func drop_soft(forward_dir: Vector3, drop_parent: Node) -> void:
	if _rb == null:
		return
	if InstructionsOverlay.is_open():
		InstructionsOverlay.hide_instructions()
