class_name Character05Bake
extends RefCounted
## Utilidad para construir la biblioteca de animaciones del NPC del supermercado.
...
static func bake_animation_into_library(library: AnimationLibrary, anim_name: String) -> bool:
	...
	if not ANIMATION_SOURCES.has(anim_name):
		push_warning("Character05Bake: animación desconocida '%s'." % anim_name)
		return false
	var config: Dictionary = ANIMATION_SOURCES[anim_name]
	var anim := _load_mixamo_animation(config)
	if anim == null:
		push_warning("Character05Bake: no se pudo cargar '%s'." % anim_name)
		return false
