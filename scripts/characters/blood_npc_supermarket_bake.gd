class_name BloodNpcSupermarketBake
extends RefCounted
## Biblioteca de animaciones del Blood NPC Supermarket (Character_05 + rig Mixamo).
...
const ANIMATION_SOURCES: Dictionary = {
	"idle_supermarket": {
		"fbx": "res://assets/characters/npc_supermarket/animations/Idle_Supermarket.fbx",
		"loop": true,
		"lock_hips": true,
	},
	"reaching_out_blood": {
		"fbx": "res://assets/characters/npc_supermarket/animations/Reaching_Out_Blood_NpcSupermarket.fbx",
		"loop": false,
		"lock_hips": true,
	},
	"vampiric_bite": {
		"fbx": "res://assets/characters/npc_supermarket/animations/Vampiric_Bite_NpcSupermarket.fbx",
		"loop": false,
		"lock_hips": true,
	},
	"vampiric_bite_slow": {
		"fbx": "res://assets/characters/npc_supermarket/animations/Vampiric_Bite_Slow_NpcSupermarket.fbx",
		"loop": false,
		"lock_hips": true,
	},
}
...
static func bake_animation_into_library(library: AnimationLibrary, anim_name: String) -> bool:
	...
	if not ANIMATION_SOURCES.has(anim_name):
		push_warning("BloodNpcSupermarketBake: animación desconocida '%s'." % anim_name)
		return false
	var config: Dictionary = ANIMATION_SOURCES[anim_name]
	var anim := _load_mixamo_animation(config)
	if anim == null:
		push_warning("BloodNpcSupermarketBake: no se pudo cargar '%s'." % anim_name)
		return false
